...
public abstract class BlockFactory {
	...
	private static BlockEffect selectEffect() {
		int alea = (int) (Math.random() * ((15 - 0)) + 1);
		if (alea <= 3) {
			return BlockEffect.GIVE_FIRE;
		}
		if (alea <= 6) {
			return BlockEffect.GIVE_POISON;
		}
		if (alea <= 9) {
			return BlockEffect.GIVE_ICE;
		}
		if (alea <= 15) {
			return BlockEffect.GIVE_MONEY;
		}
		else 
			return BlockEffect.GIVE_INVULNERABILITY;
	...
	public static MotionLess createPipe(Position position) {
		Pipe block = new Pipe(Permeability.NONE,position);
		return block;
	}
	
	public static MotionLess createUpperPipe(Position position) {
		Pipe block = new Pipe(Permeability.TELEPORTING,position);
		return block;
	}
	
	public static MotionLess createFireOctopus(Position position) {
		Octopus block = new Octopus("FireOctopus", BlockEffect.GIVE_FIRE,position);
		return block;
	}
	
	public static MotionLess createIceOctopus(Position position) {
		Octopus block = new Octopus("IceOctopus", BlockEffect.GIVE_ICE,position);
		return block;
	}
		
	public static MotionLess createPoisonOctopus(Position position) {
		Octopus block = new Octopus("PoisonOctopus", BlockEffect.GIVE_POISON,position);
		return block;
	}
